...
class Yys:
    def __init__(self, times=maxsize):
        ...
        self.setTimes = times   # 设置刷取次数, 默认无限次
        ...
        self.dir_path = ""
        self.timeout = 100  # 等待图片超时参数，更具部分可按需修改
        # 修改配置参数以添加副本模式
        ...
        keyboard.add_hotkey('q', self.__on_q_press)
        keyboard.add_hotkey('r', self.__restart)
    ...
    def setMod(self, target):
        while 1:
            if target:
                self.dir_path = self.mods_dir.get(target)
                if target != '0':
                    self.istalk = True
                break
            else:
    ...
    def start(self):
        ...
        while 1 and self.times < self.setTimes:     # 超时退出
            ...
            for url in self.lis_png_url:
                ...
                print(f"实际点击 {x = }, {y = }")   # 实际点击位置


            self.times+=isFull   # 记录副本总次数
            current_times+=isFull  # 记录未休息次数

            print(f"共已执行 {self.times} 次，总剩余 {self.setTimes-self.times} 次。上一次休息已经过 {current_times} 次，距离下一次休息 {self.rect_times - current_times} 次")
            if self.__rest(current_times): # 未休息次数是否匹配因休息次数，30 - 50次时休息
                ...
                for t in range(0, rect):
                    print(f"执行休息，已经休息 {t}s, 剩余 {rect - t}s", flush=True)
                    sleep(1)
                if self.istalk:
                    print('非活动模式，进入聊天')
                    self.__talk()
    ...
    def __total(self):
        print(f'执行结束，共计 {self.times} 次，用时 {int(time() - self.start_time)}s ')
...
def main():
    times = int(input("set times >>: "))

    test = Yys(times)

    target = input(f"select mod {test.getMod()} ('q' to exit)>>: ")

    test.setMod(target)   # 设置模式
    # test.setTimes() # 设置次数，时间等
    test.start()
...
if __name__ == '__main__':
    ...
    # 待加强，多建立不同类函数，实现不同副本的自动化，不同副本图标放在不同文件夹下
    # 计数计时，累计，自动休眠，
    # 应该把所有可能自定义配置项放入josn文件，方便修改